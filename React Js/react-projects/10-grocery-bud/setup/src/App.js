...
function App() {
  ...
  const handleSubmit = (e) => {
    ...
    if(!name) {
      ...
      showAlert(true, 'Item added in the grocery list', 'success')
      setName('')
    }
  }
  ...
  const editItem = (id) => {
    const spedificItem = list.find((item) => item.id === id);
    setIsEditing(true);
    setEditID(id)
    setName(spedificItem.title)
  }
  ...
  return <> (
      <section className='section-center'>
        ...
        <div className="form-control">
          ...
          value = {name} placeholder='e.g. Banana' 
